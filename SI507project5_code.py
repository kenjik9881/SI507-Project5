...
if __name__ == "__main__":
    ...
    # # Invoke functions---find audio posts
    tumblr_audio_search_baseurl = "https://api.tumblr.com/v2/blog/thesongsyouusedtolove.tumblr.com/posts/audio"
    tumblr_params = {'tag': "98 Degrees", 'limit': 10}

    tumblr_audioresult = get_data_from_api(tumblr_audio_search_baseurl,"Tumblr", tumblr_params) # Default expire_in_days
    tumblr_response_list2 = tumblr_audioresult['response']['posts']
    # pprint.pprint(tumblr_response_list2)
    song_tuples = []
    for apost in tumblr_response_list2:
        track_name = apost['track_name']
        num_plays = apost['plays']
        post_url = apost['post_url']
        song_tuples.append((track_name, num_plays,post_url))

    with open("98degreesaudio.csv", 'w', newline='') as f:
            writer = csv.writer(f)
            f.write("Track Name, Number of Plays, Post URL\n")
            for obj in song_tuples:
    		        writer.writerow(obj)

    # Invoke functions---find posts with the tag of 98 degrees
    tumblr_post_search_baseurl = "https://api.tumblr.com/v2/tagged"
    tumblr_post_params = {'tag': "98 Degrees", 'limit': 10}


    tumblr_tagresult = get_data_from_api(tumblr_post_search_baseurl,"Tumblr", tumblr_post_params) # Default expire_in_days
    tumblr_response_list1 = tumblr_tagresult['response']
    # pprint.pprint(tumblr_response_list1)
    tumblr_response_rows = [(arow['blog_name'],arow['post_url'], arow['tags'], arow['liked']) for arow in tumblr_response_list1]
    # pprint.pprint(tumblr_response_rows[0][2])

    with open("98degreestags.csv", 'w', newline='') as f:
            writer = csv.writer(f)
            f.write("Blog Name, Blog Post URL, All tags, Liked the post?\n")
            for obj in tumblr_response_rows:
    		        writer.writerow([obj[0], obj[1], ', '.join(obj[2]), obj[3]])

# class NinetyEightDegreesTags(object):
#     def __init__(self, atuple):
#         self.blog_name = atuple[0]
#         self.post_url = atuple[1]
#         self.tags = atuple[2]
#         self.liked = atuple[3]
#
#     def __str__(self):
#         return "The blog is {}, accessible @ {}, with tags of {}, and this is whether I liked it: {}".format(self.blog_name, self.post_url, self.tags, self.liked)
#
# anexample = NinetyEightDegreesTags(tumblr_response_rows[0])
# print(anexample)
